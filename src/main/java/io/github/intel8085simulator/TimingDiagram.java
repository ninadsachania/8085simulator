...
public class TimingDiagram extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jPanel1Layout.setHorizontalGroup(
                jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addGap(0, 1100, Short.MAX_VALUE)
        );
        jPanel1Layout.setVerticalGroup(
                jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addGap(0, 511, Short.MAX_VALUE)
        );
...
class TimingPaint extends JPanel {
    ...
    private void cycleDecoder(char choice, String address, String opcode) {
        if (choice == 'F') {
            opcodeFetch(x, address, opcode);
            x += 400;
        } else if (choice == 'S') {
            opcodeFetch6State(x, address, opcode);
            x += 600;
        } else if (choice == 'R') {
            memRead(x, address, opcode);
            x += 300;
        } else if (choice == 'W') {
            memWrite(x, address, opcode);
            x += 300;
        } else if (choice == 'I') {
            ioRead(x, address, opcode);
            x += 300;
        } else if (choice == 'O') {
            ioWrite(x, address, opcode);
            x += 300;
        } else if (choice == 'B') {
            busIdle(x);
            x += 100;
        } else if (choice == 'H') {
            memRead(x, o.engine.Dec2Hex(o.matrix.SP - 1), opcode);
            x += 300;
        } else if (choice == 'P') {
            memRead(x, o.engine.Dec2Hex(o.matrix.SP - 2), opcode);
            x += 300;
        }
